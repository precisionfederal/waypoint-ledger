/* POST /api/map — AI reads the story; the table prices it.
   ...
   Providers, in order: Anthropic (when ANTHROPIC_API_KEY is a Pages secret),
   then Cloudflare Workers AI (the AI binding). Neither reachable → the rules'
...
export async function readWithModel(env, story, debug = false) {
  ...
  const provider = env.ANTHROPIC_API_KEY ? askAnthropic : env.AI ? askWorkersAi : null;
